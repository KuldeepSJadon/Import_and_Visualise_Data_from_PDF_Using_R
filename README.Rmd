...
```{r, resetting_names}
NPI_table[NPI_table == "Central African Rep."] <- "Central African Republic"
...
Some of the countries are not matching up. It's easy enough to do some checking and replace values and join again though.
...
```{r, plot2}
ggplot(global_df2, aes(long, lat)) +
  geom_polygon(aes(group = group, fill = Rank.y))
```
